...
<body class="bg-gray-100 min-h-screen p-8">
    <div class="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
        ...
        <div id="login-section" class="mb-8">
            ...
            <p class="text-xs text-gray-500 mb-2">Enter your ADMIN_SECRET to access the dashboard.</p>
    ...
    <script>
            ...
            try {
                ...
                const expectedSettings = {
                    ...
                    'Telegram Owner Channel': [
                        {
                            key: 'ACTIVE_MESSAGING_PLATFORM',
                            required: false,
                            description: 'Which messaging platform is active',
                            format: 'whatsapp, telegram, or both (default: both)',
                            whereToGet: 'Choose the channel you want Signal Room to accept messages from.',
                            usedFor: 'Enables WhatsApp, Telegram, or both channels for owner and regular wallet users.'
                        },
                        {
                            key: 'TELEGRAM_BOT_TOKEN',
                            required: false,
                            description: 'Telegram bot token for the owner channel',
                            format: 'Token from @BotFather, for example 123456:ABC... ',
                            whereToGet: 'Telegram -> @BotFather -> /newbot, then copy the bot token.',
                            usedFor: 'Lets the owner submit Instagram links and review Viral Radar drafts in Telegram.'
                        },
                        {
                            key: 'OWNER_TELEGRAM_CHAT_ID',
                            required: false,
                            description: 'Telegram chat ID allowed to use owner controls',
                            format: 'Numeric Telegram chat ID',
                            whereToGet: 'Message your bot, then inspect the update received by /api/telegram, or use a trusted chat-ID helper bot.',
                        ...
                        },
                        {
                            ...
                            format: 'Random secret text',
                            whereToGet: 'Create a long random secret yourself and use the same value when registering the Telegram webhook.',
                            usedFor: 'Adds an extra request-level check before the owner chat ID is evaluated.'
                        }
                    ],
                    'AI': [
                        {
        ...
        function updateRequiredSettingsSummary(settings) {
            const requiredSettings = [
                ...
                {key: 'AI_PROVIDER', description: 'AI Provider (openai or anthropic)'}
            ];
